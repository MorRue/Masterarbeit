...
toInvestigate_a = [4 , 22 , 64 , 150 , 274 , 462 , 748 , 1066 , 1522]
toInvestigate_t = [2 ,  4 ,  6 ,   8 ,  10 ,  12 ,  14 ,   16 ,   18]




path = f"../Investigate Specific/Vector Parabolas/"

os.mkdir(path)

globalWriter,file = csvStuff.createWriter(path+"LogAll.csv")
header = ["a_t (sum of sums...)","t","num steps till current Peeling = 0th Peeling","vertical Period"]
...
for i in range(0,len(toInvestigate_a)):
    a = toInvestigate_a[i]
    t = toInvestigate_t[i]
    name = path+"Logs/a="+str(a)+" t="+str(t)
    print(name)
    os.mkdir(name)

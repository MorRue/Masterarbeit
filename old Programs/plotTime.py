...
def mainTwo(numX,numPeelings,plot,plotPeriod,a_one_in,a_two_in, b_one_in, b_two_in , g_one_in,g_two_in):
    ...
    mini = min(times)
    maxi = max(times)
    del(times[times.index(mini)])
    del(times[times.index(maxi)])
...
for x in toinvestigate:
    for j in range(1,101,100):
        for i in range(j,101,100):
            for l in range(0,11,11):
                for m in range(1,100001,1000):
                    ...
                    timeNeeded = mainTwo(highestx,numPeelings, plot,plotPeriod,a_one,a_two, b_one , b_two, g_one,g_two)
                    times.append(timeNeeded)
                    nums.append(highestx)


newTimes = []
for x in times:
    newTimes.append(x/(numPeelings-2))
f,(figOne) = plt.subplots(1,1)
figOne.xaxis.set_label_text("3 . Horizontale Periode")
figOne.yaxis.set_label_text("Benötigte Zeit in Sekunden")
figOne.scatter(nums,newTimes)
plt.show()
